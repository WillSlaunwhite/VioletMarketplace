...
export class UserBalanceService {
      ...
      switchMap((options) => {
        return this.http.get<UserCurrencyBalance>( `${this.url}/${userId}/balance/${currencyType}`, options)
          .pipe(
            catchError((err: any) => {
              return throwError(
                () => new Error( `UserBalanceService.getBalance: error retrieving User's balance of ${currencyType}`)
              );
            })
          )
      })
  ...
  credit( userId: number, currencyType: string, amount: number): Observable<UserCurrencyBalance> {
    ...
    return this.auth.getHttpOptions().pipe(
      switchMap((options) => {
        return this.http.post<UserCurrencyBalance>(`${this.url}/credit`, body, options).pipe(
          catchError((err: any) => {
            return throwError(
              () =>
                new Error(
                  "UserBalanceService.credit: error crediting User's balance of " +
                    currencyType
                )
            );
          })
...
debit( userId: number, currencyType: string, amount: number): Observable<UserCurrencyBalance> {
    ...
    return this.auth.getHttpOptions().pipe(
      switchMap((options) => {
        return this.http.post<UserCurrencyBalance>(`${this.url}/debit`, body, options).pipe(
          catchError((err: any) => {
            return throwError(
              () =>
                new Error(
                  "UserBalanceService.debit: error debiting User's balance of " +
                    currencyType
                )
            );
          })
  ...
  getAllBalances(userId: number): Observable<UserCurrencyBalance[]> {
    return this.auth.getHttpOptions().pipe(
      switchMap((options) =>
        this.http
          .get<UserCurrencyBalance[]>(`${this.url}/${userId}/balance`, options)
          .pipe(
            catchError((err: any) => {
              return throwError(
                () =>
                  new Error(
                    'UserBalanceService.getAllBalances: error retrieving all User balances'
                  )
              );
            })
          )
