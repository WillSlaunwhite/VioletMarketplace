...
class SearchServiceImpl(
    ...
    override fun search(query: String): SearchResults {
        ...
        val userResults =
            trieResults.filter { it.type == "User" }.map { userRepo.findById(it.id).get().toCommonDTO() }.toMutableSet()
        val tokenResults =
            trieResults.filter { it.type == "Token" }.map { tokenRepo.findById(it.id).get().toCommonDTO() }
                .toMutableSet()
